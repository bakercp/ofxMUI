...
public:
    ...
    ofxGuiIcon() : onLoaded(false), offLoaded(false)  {}
    
    ofxGuiIcon(string _img) : onLoaded(false), offLoaded(false) {
        set(_img, _img);
    }    

    ofxGuiIcon(string _on, string _off) : onLoaded(false), offLoaded(false) {
        set(_on, _off);
    }    
    ...
    void setOn(string _on) {
        ...
        if(!onLoaded) {
            ofLog(OF_LOG_ERROR, "ofxGuiIcon::Icons on Image " + _on + " not loaded.");
        }
    ...
    void setOff(string _off) {
        ...
        if(!offLoaded) {
            ofLog(OF_LOG_ERROR, "ofxGuiIcon::Icons off Image " + _off + " not loaded.");
        }
    ...
    void set(string _on, string _off) {
        ...
        if(on.width != off.width || on.height != off.height) {
            ofLog(OF_LOG_WARNING, "ofxGuiIcon::Icons (" + _on + ") & (" + _off + ") are not equal in size.  Unpredictable results.");
        }
