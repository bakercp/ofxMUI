...
//--------------------------------------------------------
void ofxMuiFont::setGlobalDpi(int newDpi){
...
static ofCharacter makeContoursForCharacter(FT_Face &face){
	...
	#include <set>
	set<ofxMuiFont*> all_fonts;
	void ofUnloadAllFontTextures(){
		set<ofxMuiFont*>::iterator it;
...
void ofxMuiFont::unloadTextures(){
	if(!bLoadedOk) return;
...
void ofxMuiFont::reloadTextures(){
	loadFont(settings);
}

void ofxMuiFont::resetMetrics() {
...
//-----------------------------------------------------------
bool ofxMuiFont::loadFont(string fontFilename, 
                              int fontSize, 
                              bool bAntiAliased, 
...
//-----------------------------------------------------------
bool ofxMuiFont::loadFont(Settings _settings) { 

    if(_settings.fontFilename.empty()) {
		ofLog(OF_LOG_ERROR,"ofxMuiFont::loadFont - No font name specified.");
		return false;
    }
    ...
    if (err){
		ofLog(OF_LOG_ERROR,"ofxMuiFont::loadFont - Error initializing freetype lib: FT_Error = %d", err);
		return false;
	}
	...
	if (err) {
        ...
        ofLog(OF_LOG_ERROR,"ofxMuiFont::loadFont - %s: %s: FT_Error = %d", errorString.c_str(), fontFilenameWithPath.c_str(), err);
		return false;
	}
	...
	for (int i = 0 ; i < nCharacters; i++){
		...
		err = FT_Load_Glyph( face, 
                            ...
                            loadMode );
        if(err){
			ofLog(OF_LOG_ERROR,"ofxMuiFont::loadFont - Error with FT_Load_Glyph %i: FT_Error = %d", i, err);
		}
        ...
        if(bHasKerning) {
            ...
            for (int j = 0 ; j < nCharacters; j++){
                ...
                if(!kerningError) {
                    ...
                    ofLog(OF_LOG_ERROR,"ofxMuiFont::loadFont - Error with FT_Get_Kerning %i/%i: FT_Error = %d", i, j, kerningError);
                }
            }
        }
...
//-----------------------------------------------------------
bool ofxMuiFont::isLoaded() {
	return bLoadedOk;
}

//-----------------------------------------------------------
bool ofxMuiFont::isAntiAliased() {
	return settings.bAntiAliased;
}

//-----------------------------------------------------------
bool ofxMuiFont::hasFullCharacterSet() {
	return settings.bFullCharacterSet;
}

//-----------------------------------------------------------
bool ofxMuiFont::hasKerning() {
	return bHasKerning;
}

//-----------------------------------------------------------
bool ofxMuiFont::hasContours() {
    return bHasContours;
}

//-----------------------------------------------------------
int ofxMuiFont::getSize() {
	return settings.fontSize;
}

//-----------------------------------------------------------
void ofxMuiFont::setLineHeight(float _newLineHeight) {
	lineHeight = _newLineHeight;
}

//-----------------------------------------------------------
float ofxMuiFont::getLineHeight(){
	return lineHeight;
}

//-----------------------------------------------------------
void ofxMuiFont::setLetterSpacing(float _letterSpacing) {
	letterSpacing = _letterSpacing;
}

//-----------------------------------------------------------
float ofxMuiFont::getLetterSpacing(){
	return letterSpacing;
}

//-----------------------------------------------------------
void ofxMuiFont::setSpaceSize(float _spaceSize) {
	spaceSize = _spaceSize;
}

//-----------------------------------------------------------
float ofxMuiFont::getSpaceSize(){
	return spaceSize;
}

//------------------------------------------------------------------
ofCharacter ofxMuiFont::getCharacterAsPoints(int character, float x, float y){
    return getCharacterAsPoints(character, ofPoint(x,y));
}

//------------------------------------------------------------------
ofCharacter ofxMuiFont::getCharacterAsPoints(int character, ofPoint p){
...
//------------------------------------------------------------------
bool ofxMuiFont::isInCharacterSet(int character) {
    return character >= NUM_CHARACTER_TO_START_ && charToIndex(character) < nCharacters;
}

//------------------------------------------------------------------
bool ofxMuiFont::isInShapeCharacterSet(int character) {
    return character >= NUM_CHARACTER_TO_START_ && charToIndex(character) < (int)charOutlines.size();
}


//-----------------------------------------------------------
ofCharacter ofxMuiFont::drawChar(int character, float x, float y, bool asShapes) {
...
//-----------------------------------------------------------
ofCharacter ofxMuiFont::drawChar(int character, ofPoint p, bool asShapes) {
    return drawChar(character,p.x,p.y,asShapes);
}

//-----------------------------------------------------------
ofCharacter ofxMuiFont::drawCharAsShapes(int character, float x, float y) {
    return drawChar(character, x, y, true);
}

//-----------------------------------------------------------
ofCharacter ofxMuiFont::drawCharAsShapes(int character, ofPoint p) {
    return drawChar(character, p.x, p.y, true);
}


//-----------------------------------------------------------
vector<ofCharacter> ofxMuiFont::getStringAsPoints(string s, float x, float y, float w, float h) {
    ofRectangle boundingBox(x,y,w,h);
    return getStringAsPoints(s,boundingBox);
}

//-----------------------------------------------------------
vector<ofCharacter> ofxMuiFont::getStringAsPoints(string s, ofRectangle boundingBox) {
...
//-----------------------------------------------------------
vector<ofCharacter>  ofxMuiFont::getCharacterOffsets(string s, float x, float y, float w, float h) {
    ofRectangle boundingBox(x,y,w,h);
    return getCharacterOffsets(s, boundingBox);
}

//-----------------------------------------------------------
vector<ofCharacter>  ofxMuiFont::getCharacterOffsets(string s, ofRectangle boundingBox) {
    return textLayout(s, boundingBox, false, false);
}

//-----------------------------------------------------------
float ofxMuiFont::stringWidth(string c) {
    float width = 0;
    int index = 0;
    int start = 0;
...
//-----------------------------------------------------------
float ofxMuiFont::_widthChar(int character) {
    ...
    if(isInCharacterSet(character)) { // is it in our char set?
        ...
        ofLogVerbose("ofxMuiFont::_widthChar character was not in set : " + ofToString((char)character));
        return 0.0f;
    }
}
...
//-----------------------------------------------------------
float ofxMuiFont::_widthString(string c, int start, int stop) {
    // assume no new lines
    float width = 0.0f;
    if(start >= 0 && stop < (int)c.length() - 1) {
       ...
       // start or stop were out of bounds
        ofLogWarning("ofxMuiFont::_widthString start / stop were out of bounds.");
    }
    return width;
}
...
//-----------------------------------------------------------
ofVec2f ofxMuiFont::getKerning(int leftChar, int rightChar) {
    ofVec2f k(0.0f,0.0f);
...
//-----------------------------------------------------------
float   ofxMuiFont::getHorizontalKerning(int leftChar, int rightChar) {
    return getKerning(leftChar, rightChar).x;
}

//-----------------------------------------------------------
float   ofxMuiFont::getVerticalKerning(int leftChar, int rightChar) {
    return getKerning(leftChar, rightChar).y;
}

//-----------------------------------------------------------
int ofxMuiFont::charToIndex(int character) {
    return (character - NUM_CHARACTER_TO_START_);
}

//-----------------------------------------------------------
int ofxMuiFont::indexToChar(int index) {
    return (index + NUM_CHARACTER_TO_START_);
}

//-----------------------------------------------------------
ofRectangle ofxMuiFont::getStringBoundingBox(vector<ofCharacter> offsets, bool shrinkToContours) {
...
//-----------------------------------------------------------
ofRectangle ofxMuiFont::getStringBoundingBox(string s, ofPoint p, bool shrinkToContours) {
    return getStringBoundingBox(s, p.x, p.y, shrinkToContours);
}

//-----------------------------------------------------------
ofRectangle ofxMuiFont::getStringBoundingBox(string s, float x, float y, bool shrinkToContours ){
...
//-----------------------------------------------------------
float ofxMuiFont::stringHeight(string s) {
    ofRectangle rect = getStringBoundingBox(s, 0,0);
    return rect.height;
}

vector<ofCharacter> ofxMuiFont::drawString(string s, float x, float y, float w, float h, bool asShapes) {
    ofRectangle boundingBox(x,y,w,h);
    return textLayout(s, boundingBox, true, asShapes);
}

//=====================================================================
vector<ofCharacter> ofxMuiFont::drawString(string s, ofRectangle boundingBox, bool asShapes) {
    return textLayout(s, boundingBox, true, asShapes);
}


//=====================================================================
void ofxMuiFont::drawString(vector<ofCharacter> offsets, ofVec2f offset, bool asShapes) {
    ofPushMatrix();
    ofTranslate(offset);
...
//=====================================================================
vector<ofCharacter> ofxMuiFont::drawStringAsShapes(string s, float x, float y, float w, float h) {
    ofRectangle boundingBox(x,y,w,h);
    return textLayout(s, boundingBox, true, true);
}

//=====================================================================
vector<ofCharacter> ofxMuiFont::drawStringAsShapes(string s, ofRectangle boundingBox) {
    return textLayout(s, boundingBox, true, true);
}

//=====================================================================
void ofxMuiFont::drawStringAsShapes(vector<ofCharacter> offsets, ofVec2f offset) {
    return drawString(offsets,offset,true);
}

//=====================================================================
void ofxMuiFont::_drawChar(int character, ofPoint p, bool asShapes) {
...
//-----------------------------------------------------------
vector<ofCharacter>  ofxMuiFont::textLayout(string s, 
                                              ofRectangle bb,
                                              bool draw,
                                              bool drawShapes) {
    ...
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::drawString - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    	return offsets;
    }
    
	if (drawShapes && !bHasContours){
		ofLog(OF_LOG_ERROR,"ofxMuiFont::drawStringAsShapes - Error : contours not created for this font - call loadFont with makeContours set to true");
		return offsets;
	}
...
//-----------------------------------------------------------
int ofxMuiFont::getNumCharacters() {
	return nCharacters;
}

//-----------------------------------------------------------
string ofxMuiFont::getFamilyName() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getFamilyName - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return fontFamilyName;
}
//-----------------------------------------------------------
string ofxMuiFont::getStyleName() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getStyleName - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return fontStyleName;
}
//-----------------------------------------------------------
int ofxMuiFont::getNumFacesInFont() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getNumFacesInFont - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return numFacesInFont;
}
//-----------------------------------------------------------
int ofxMuiFont::getCurrentFaceIndex() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getCurrentFaceIndex - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return currentFaceIndex;
}
//-----------------------------------------------------------
int ofxMuiFont::getMaxAdvanceWidth() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getMaxAdvanceWidth - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return maxAdvanceWidth;
}
//-----------------------------------------------------------
int ofxMuiFont::getMaxAdvanceHeight() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getMaxAdvanceHeight - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return maxAdvanceHeight;
}
//-----------------------------------------------------------
int ofxMuiFont::getUnderlinePosition() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getUnderlinePosition - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return underlinePosition;
}
//-----------------------------------------------------------
int ofxMuiFont::getUnderlineThickness() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getUnderlineThickness - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return underlineThickness;
}
//-----------------------------------------------------------
int ofxMuiFont::getAscender() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getAscender - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return ascender;
}
//-----------------------------------------------------------
int ofxMuiFont::getDescender() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getDescender - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return descender;
}
//-----------------------------------------------------------
int ofxMuiFont::getHeight() {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofxMuiFont::getHeight - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    }
    return height;
}
