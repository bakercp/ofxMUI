...
//-----------------------------------------------------------
bool ofTrueTypeFont::loadFont(string filename, int fontsize, bool _bAntiAliased, bool _bFullCharacterSet, bool makeContours, float simplifyAmt, int dpi, bool forceAutoHinting) {
    ...
    FT_Int32 loadMode = forceAutoHinting ? FT_LOAD_DEFAULT : FT_LOAD_TARGET_LIGHT;
    FT_Render_Mode renderMode = bAntiAliased ? FT_RENDER_MODE_NORMAL : FT_RENDER_MODE_MONO;
	//--------------------- load each char -----------------------
	for (int i = 0 ; i < nCharacters; i++){

		//------------------------------------------ anti aliased or not:
		err = FT_Load_Glyph( face, 
                            FT_Get_Char_Index( face, (unsigned char)(i+NUM_CHARACTER_TO_START) ), 
                            loadMode );
        if(err){
...
ofTTFCharacter ofTrueTypeFont::getCharacterAsPoints(int character){
	...
	if( bMakeContours && 
       (int)charOutlines.size() > 0 && 
       character >= NUM_CHARACTER_TO_START && 
       character - NUM_CHARACTER_TO_START < (int)charOutlines.size()){
...
//-----------------------------------------------------------
int ofTrueTypeFont::getCharPropertyIndex(int c) {
    int cy = c - NUM_CHARACTER_TO_START;
    if (cy < nCharacters){ 			// full char set or not?
        return cy;
    } else {
        //ofLog(OF_LOG_ERROR,"ofTrueTypeFont::getCharProperty - char out of range");
    	return -1;
    }
}

//-----------------------------------------------------------
charProps ofTrueTypeFont::getCharProp(int c) {
    int index = getCharPropertyIndex(c);
    charProps cp;
    if(index > -1) {
        cp = cps[index];
    }
...
//-----------------------------------------------------------
float ofTrueTypeFont::charWidth(int c) {
    if (!bLoadedOk){
    	ofLog(OF_LOG_ERROR,"ofTrueTypeFont::charWidth - font not allocated");
    	return 0;
    }
    ...
    if(index > -1) {
        cp = cps[index];
        if(c == ' ') {
            int cy = getCharPropertyIndex('p');
            return cps[cy].width * letterSpacing * spaceSize;
            // zach - this is a bug to fix -- for now, we don't currently deal with ' ' in calculating string bounding box
        } else {
            //return cp.setWidth * letterSpacing;   
            return cp.width * letterSpacing;
        }
...
ofRectangle ofTrueTypeFont::getStringBoundingBox(string c, float x, float y){
    ...
    if ( len < 1 || cps.empty() ) return myRect;

    while(index < len) {
        int cy = getCharPropertyIndex(c[index]);
        if (cy > -1){ 			// full char set or not?
            charProps cp = cps[cy];
            if (c[index] == '\n') {
                Y += lineHeight;
				X = 0 ; //reset X Pos back to zero
            } else if (c[index] == ' ') {
                cy = getCharPropertyIndex('p');
                X += cps[cy].width * letterSpacing * spaceSize;
            } else {
		...
		if (cy < nCharacters){ 			// full char set or not?
		  if (c[index] == '\n') {
				...
				X = 0 ; //reset X Pos back to zero
				 ...
				 int cy = (int)'p' - NUM_CHARACTER_TO_START;
				 X += cps[cy].width; // * letterSpacing * spaceSize // TODO
				 //glTranslated(cps[cy].width, 0, 0);
		  } else {
				drawCharAsShape(cy, X, Y);
				X += cps[cy].setWidth; // * letterSpacing; // TODO
